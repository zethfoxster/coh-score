...
#define TOGGLEVAR(func, var) static void handle_##func(int argc, char **argv) { \
	var = !var; \
	printf(#func ": %s\n", var ? "ON" : "OFF"); \
}
...
static void handle_chanlist(int argc, char **argv)
{
	if (argc >= 1)
		sendShardCmd("chanlist", "%s", argv[1]);
}
...
static void handle_gotopos(int argc, char **argv)
{
	Vec3 dest;
	if (argc == 4) {
		dest[0] = atof(argv[1]);
...
static void handle_enterdoor(int argc, char **argv) {
	Entity *e = playerPtr();
	enterDoorClient(ENTPOS(e), "0");
}

TOGGLEVAR(notimeout, control_state.notimeout)
TOGGLEVAR(selectanyentity, game_state.select_any_entity)

#define CMD(cmd)	if (stricmp(keyword, #cmd) == 0) { handle_##cmd(argc, argv); return 1; }

int textClientCmdParse(char *cmdx)
{
	char *cmd;
	char *argv[100];
	int argc;
