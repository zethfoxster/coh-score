...
void fxPreloadFxInfo()
{
	...
	ParserLoadFiles(dir, filetype, persistFilename, flags, ParseFxInfoList, &fx_infolist, NULL, NULL, (ParserLoadPreProcessFunc)fxPreloadFxInfoPreProcess, NULL);
#endif
...
int fxInitFxInfo(FxInfo* fxinfo)
{
	...
	strcpy( usedInputs[1], "Target" );
	strcpy( usedInputs[2], "PrevTarget" );
	usedInputsCount = 3;
		...
		//Get inputs that are actually used
		{
			...
			{ 
				...
				if( 0 == stricmp( input, "Origin" ) )
					...
					okInput = 1;
				else if( 0 == stricmp( input, "PrevTarget" ) )
					okInput = 1;
				else if( 0 == stricmp( input, "P_Root" ) )
					okInput = 1;
				else if( 0 == strnicmp( input, "T_", 2 ) )
					okInput = bone_NameIsValid( input + 2 );
				else if( 0 == strnicmp( input, "C_", 2 ) )
					okInput = bone_NameIsValid( input + 2 );
				else if( 0 == strnicmp( input, "P_", 2 ) )
					okInput = bone_NameIsValid( input + 2 );
				else
			...
			for( i = 1 ; i < fxinfo->inputsRealCount ; i++ )
			{
				if( 0 == stricmp( "Origin", fxinfo->inputsReal[i] ) )
				{
					//Swap origin to the top
					char * temp;
					temp = fxinfo->inputsReal[i];
					fxinfo->inputsReal[i] = fxinfo->inputsReal[0];
					fxinfo->inputsReal[0] = temp;
				}
			}
			...
			//And an even dumber one for PrevTarget
			for( i = 0 ; i < fxinfo->inputsRealCount ; i++ )
			{
				if( i != 2 && 0 == stricmp( "PrevTarget", fxinfo->inputsReal[i] ) )
				{
					//Swap prevtarget for position 2
					char * temp;
					temp = fxinfo->inputsReal[i];
					fxinfo->inputsReal[i] = fxinfo->inputsReal[2];
					fxinfo->inputsReal[2] = temp;
				}
			}
		}
