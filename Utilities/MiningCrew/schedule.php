...
else if(@$_POST['edit'] && $id)
{
	...
	{
		$frequency = (int)@$_POST['frequency'] ? (int)$_POST['frequency'] : (int)@$_POST['frequency_other'];
		...
		(@$_GET['a'] == 'n' && ($schedule || @(int)$_GET['m'])) )
{
	...
	if($collected <= 1)
		echo "<div class='formrow'><div class='label'>Frequency</div>".select_frequency('frequency',$frequency)."</div>\n";
	if(!$collected)
