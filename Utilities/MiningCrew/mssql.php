...
// returns a reference to be passed to other functions for the parameter $db
{
	...
	if($link === false || !@mssql_select_db($db,$link))
	{
		if($fail)
			sql_fatal_error("Could not connect to database", $mssql_db);
		else
			return false;
	}
...
//echo "<div class='mssql_error'>SQL Query<br>".($query ? "<span class='mssql_error_query'>".nl2br($query)."</span><br>" : "")."<div class='mssql_error_trace'>\n\n".error_back_trace(2)."</div></div>";
	...
	if($db===false || !mssql_select_db($mssql_dbs[$db]['db'],$mssql_dbs[$db]['link']))
		sql_fatal_error("Could not select database.", $mssql_dbs[$db]);
