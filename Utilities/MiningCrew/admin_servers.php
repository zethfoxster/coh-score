...
else if(@$_POST['editserver'] && $id)
{
	...
	sql_query("UPDATE servers SET "
		...
		."logindb=".sql_string($_POST['database'])
		." WHERE id=$id");
...
else if(@$_POST['newserver'])
{
	if(sql_db_connect($_POST['address'],$_POST['user'],$_POST['password'],$_POST['database']))
	{
		...
		$query .= "INSERT INTO servers (handle,name,loginaddr,loginuser,loginpass,logindb) VALUES ("
			."@handle, "
			."$name, "
...
else if(($server && @$_GET['a'] == 'e') || @$_GET['a'] == 'n')
{
	...
	if($server)
	{
		...
		$database = htmlspecialchars($server['logindb'],ENT_QUOTES);
		$hide = $group['hide'];
	}
	else
	{
