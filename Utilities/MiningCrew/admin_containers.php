...
if(@$_POST['edit'] && $table)
{
	...
	foreach(new QueryIterator("SELECT * FROM containers WHERE container=".sql_string($container)." AND tablename LIKE ".sql_string("$table%")) as $k => $v)
	{
		...
		$dataok = (@$_POST['dataok'][$k] && $v['columntype'] != 'attribute') ? 1 : 0;
		$whereok = @$_POST['whereok'][$k] ? 1 : 0;

		sql_query("UPDATE containers SET name=$name, groupok=$groupok, dataok=$dataok, whereok=$whereok WHERE id=$k");
	}
...
else if($container && @$_POST['type'] == 'mineacc')
{
	...
	foreach(new QueryIterator("SELECT servers.* FROM serverhandles, serverlookup, servers WHERE hide=0 AND serverlookup.handle=serverhandles.id AND serverlookup.server=servers.id") as $server)
		foreach(new QueryIterator("SELECT DISTINCT MinedData.Field FROM MiningAccumulator, MinedData WHERE MiningAccumulator.Name LIKE ".sql_string("$container.%")." AND MiningAccumulator.ContainerId = MinedData.ContainerId ORDER BY MinedData.Field",sql_server_connect($server,true)) as $v)
...
if($table)
{
	...
	$parselevel = false;
	echo "<table>\n";
	echo "<tr><th class='rowt' colspan=6>$container\n";
	foreach(new QueryIterator("SELECT * FROM containers WHERE container=".sql_string($container)." AND tablename LIKE ".sql_string("$table%")." ORDER BY tablename") as $v) // Form handling should match this query
	{
		...
		if($v['tablename'] != $tablename)
		{
			...
			if($table != 'mineacc')
				echo "<tr><th class='rowh' colspan=6>$tablename\n";
			else if(substr($tablename,7))
				$def = substr($tablename,7);
			echo "<tr><th class='rowh'>Display Name<th class='rowh'>Column Name<th class='rowh'>Type<th class='rowh'>Data<th class='rowh'>Group<th class='rowh'>Where\n";
		}
		echo "<tr>";
		...
		if($v['columntype'] == 'attribute' || $v['columntype'] == 'string' ||
			($table == 'mineacc' && $tablename != 'mineacc'))
			echo "<td class='row$row'>&nbsp;";
		else
			echo "<td class='row$row'><input type=checkbox name='dataok[$v[id]]'".($v['dataok']?' checked':'')."><span class='description'>D</span>";
		if($tablename == 'mineacc' && $v['columnname'] != 'Name')
			echo "<td class='row$row'>&nbsp;";
		else
			echo "<td class='row$row'><input type=checkbox name='groupok[$v[id]]'".($v['groupok']?' checked':'')."><span class='description'>G</span>";
		if($table == 'mineacc') // FIXME: constraints for mineaccs
			echo "<td class='row$row'>&nbsp;";
		else
			echo "<td class='row$row'><input type=checkbox name='whereok[$v[id]]'".($v['whereok']?' checked':'')."><span class='description'>W</span>";
		echo "\n";
		if(trim($v['description']))
			echo "<tr><td colspan=6 class='row$row description' style='font-style:italic'>$v[description]\n";
		$row = $row ? 0 : 1;
	}
	echo "<tr><td class='rowf' colspan=6>\n";
	if($table == 'mineacc')
	...
	echo "    <div class='formrow'><input type=submit class='butwarning' name='edit' value='Adjust Container Fields'></div>\n";
	if($table == 'mineacc')
	{
		echo "<tr><td class='rowf' colspan=6><input type=hidden name='olddef' value=\"".htmlspecialchars($def)."\">\n";
