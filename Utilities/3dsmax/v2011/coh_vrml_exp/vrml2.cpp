...
static Point3 getNormal(Mesh &mesh, int index, int corner)
{
	...
	if (meshNorm) {
		int normalIndex(meshNorm->GetNormalIndex(index, corner));
		if (normalIndex >= 0 && meshNorm->GetNormalExplicit(normalIndex))
		{
...
//			return meshNorm->Normal(normID).Normalize();
	...
	if((norCnt = (int)(rv->rFlags & NORCT_MASK)) && smGroup) {
		if (norCnt == 1)
			return rv->rn.getNormal();
...
VRML2Export::VrmlOutFileInfo()
{
    ...
    fprintf(mStream, _T("# Produced by 3D Studio MAX VRML97 exporter, Version %.5g, Revision %.5g Paragon Version 7\n"),
        vernum, betanum);
...
VRML2Export::GenerateUniqueNodeNames(INode* node)
{
	...
	if (!nList->hasName) {
	 // take mangled name and get a unique name
		...
		{
			TCHAR newname[512];
			strcpy(newname, node->GetName());
			strcat(newname, "_HIDDEN");
