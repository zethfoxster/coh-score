...
namespace FxLauncher
{
    ...
    public partial class FXLauncherForm : Form
    {
        ...
        public FXLauncherForm()
        {
            this.Icon = Icon.FromHandle(Properties.Resources.fxLauncherIcon.GetHicon());
            ...
            tfxPath = @"C:\Cryptic\scratch\tfx.txt";

            favPath = @"C:\Cryptic\scratch\myFavorite.txt";

            InitializeComponent();
            
            imgList = new ImageList();
            imgList.Images.Add(Properties.Resources.CLSDFOLD);
            imgList.Images.Add(Properties.Resources.OPENFOLD);
            imgList.Images.Add(Properties.Resources.DOCR);
        ...
        {
            string root = @"c:\gamefix\";

            if (gamePath != null && gamePath.ToLower().Contains(@"c:\gamefix2\"))
                root = @"c:\gamefix2\";

            else if (gamePath != null && gamePath.ToLower().Contains(@"c:\game\"))
                root = @"c:\game\";
        ...
        private void loadMap(string map, string locationStr)
        {
            if (fx_Handle == IntPtr.Zero)
            {
                MessageBox.Show("FX Editor is not running.");
                return;
            }
            ...
            IntPtr mHwd = this.Handle;

            SetForegroundWindow(mHwd);
            this.BringToFront();
            this.Focus();

            DialogResult dr = MessageBox.Show(this, "Did the map finish loading?", "Map Loading..", MessageBoxButtons.YesNo);
            this.Focus();
        ...
        {
            tabControl1.Controls.Add(mapTabpage);
            mapsTreeView.BeginUpdate();
            mapsTreeView.Nodes.Clear();
            ArrayList tfxList = new ArrayList();
            fillTfxList(@"c:\temp\loadmaps.txt", tfxList);

            TreeNode tn = null, tnCh = null;
            string[] mapLoc = new string[2];

            foreach (object obj in tfxList)
            {
                string file = (string)obj;
                if (file.Length > 2)
                {
                    if (file.ToLower().StartsWith("Portal".ToLower()) ||
                        file.ToLower().StartsWith("Teleporter".ToLower()))
                    {
                        tnCh = new TreeNode(file, 2, 2);
                        tn.Nodes.Add(tnCh);
                    }
                    else if (file.ToLower().StartsWith("loadmap".ToLower()))
                    {
                        mapLoc[0] = file;
                    }
                    else if (file.ToLower().StartsWith("setpospyr".ToLower()))
                    {
                        mapLoc[1] = file;
                        tnCh.Tag = mapLoc;
                    }
        ...
        private void fillGameTree()
        {
            if (gamePath != null)
            {
                TreeNode gamefxTN = new TreeNode("FX");
                gamefxTN.Tag = getRoot() + @"data\fx";
                gamefxTN.Name = "FX";
            ...
            {
                TreeNode gamefxTN = new TreeNode("FX");
                gamefxTN.Tag = @"c:\game\data\fx";
                gamefxTN.Name = "FX";
                treeView1.Nodes.Add(gamefxTN);
                fillTree(@"c:\game\data\fx", treeView1, gamefxTN);

                TreeNode gameFIXfxTN = new TreeNode("FX");
                gameFIXfxTN.Tag = @"c:\gamefix\data\fx";
                gameFIXfxTN.Name = "FX";
                treeView2.Nodes.Add(gameFIXfxTN);
                fillTree(@"c:\gamefix\data\fx", treeView2, gameFIXfxTN);
        ...
        private void fillTfxList(string path, ArrayList tfxList)
        {
            ...
            if(tfxList.Count>0 && path.Contains("tfx.txt"))
        ...
        private void buildFxContents()
        {
            ...
            {
                string root = getRoot();
                string path = root + @"data\fx";

                list = Directory.GetFiles(path, "*.fx", SearchOption.AllDirectories);
            }
            catch (Exception e)
            {
                MessageBox.Show(e.Message);
        ...
        {
            if (parseTree_menuItem.Tag != null)
            {
