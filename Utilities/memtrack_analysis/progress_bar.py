...
#
# Hypnos improvements to minimize impact of updates:
#    Only process when display would change
#    Auto print mode
#These make it easier to use from clients w/ mimial impact
...
class ProgressBar:
    def __init__(self, min_value = 0, max_value = 100, width=77,**kwargs):
        ...
        self.auto_print = kwargs.get('autoprint', False)
        ...
        self.width = width
        self.shown_hashes = -1   # current number of hashes displayed
        self.shown_percent = -1  # current percentage displayed
        self.amount = 0       # When amount == max, we are 100% done
        self.factor = (self.width - 2)/float(self.span)
    ...
    def build_bar(self):
        ...
        num_hashes = int(round(diff*self.factor))
        percent_done = int(round((diff / float(self.span)) * 100.0))
        
        if self.shown_hashes != num_hashes or self.shown_percent != percent_done:

          self.shown_hashes = num_hashes
          self.shown_percent = percent_done
          ...
          if self.mode == 'dynamic':
              ...
              self.bar = self.char * num_hashes + ' ' * (self.width - 2 - num_hashes)
...
def main():
    ...
    print 'Example 1: Fixed Bar, Auto Print'
    prog = ProgressBar(0, limit, 77, mode='fixed', autoprint="true")
