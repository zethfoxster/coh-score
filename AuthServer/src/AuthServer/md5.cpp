...
void MD5::Update (unsigned char *input, unsigned int inputLen)
{
    ...
    if ((count[0] += ((DWORD)inputLen << 3)) < ((DWORD)inputLen << 3))
        count[1]++;
    count[1] += ((DWORD)inputLen >> 29);
...
*/
  MD5_memset ((POINTER)this, 0, sizeof (MD5));
}
...
/* Decodes input (unsigned char) into output (DWORD). Assumes len is
  a multiple of 4.
 */
static void Decode (DWORD *output, unsigned char *input, unsigned int len)
{
  unsigned int i, j;
