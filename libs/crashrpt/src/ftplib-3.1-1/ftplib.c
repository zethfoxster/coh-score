...
static int socket_wait(netbuf *ctl)
{
	...
	do
	{
		...
		tv = ctl->idletime;
		//! @note Casting ctl->handle to 32 bits.  But this is fine, since according to the MSDN, that parameter is ignored.
		...
		if (rv == -1)
		{
			...
			strncpy_s(ctl->ctrl->response, sizeof(ctl->ctrl->response),
				error, _TRUNCATE);
			break;
		}
		else if (rv > 0)
...
static int readline(char *buf,int max,netbuf *ctl)
{
	...
	do
	{
		if (ctl->cavail > 0)
		{
			...
			if (end != NULL)
			{
				// Assume x's value fits into 32 bits.
				x = (int) (end - bp);
			}
			retval += x;
			bp += x;
...
static int FtpAcceptConnection(netbuf *nData, netbuf *nControl)
{
	...
	if (i < nData->handle)
		i = nData->handle;
	//! @note Casting i to 32 bits.  But this is fine, since according to the MSDN, that parameter is ignored.
	i = select((int)i+1, &mask, NULL, NULL, &tv);
	if (i == -1)
	{
		char error[80];
		strerror_s(error, sizeof(error), errno);
		strncpy_s(nControl->response, sizeof(nControl->response),
			error, _TRUNCATE);
	...
	else
	{
		if (FD_ISSET(nData->handle, &mask))
		{
			...
			{
				char error[80];
				// Assume i fits into 32 bets.
				strerror_s(error, sizeof(error), (int)i);
				strncpy_s(nControl->response, sizeof(nControl->response), error, _TRUNCATE);
				nData->handle = 0;
				rv = 0;
			}
		}
...
GLOBALDEF int FtpModDate(const char *path, char *dt, int max, netbuf *nControl)
{
	...
	if (!FtpSendCmd(buf,'2',nControl))
		...
		strncpy_s(dt, max, &nControl->response[4], _TRUNCATE);
	return rv;
}
