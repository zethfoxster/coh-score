...
BOOL CMailMsg::MAPISend()
{
   ...
   if (m_bReady || Initialize())
   {
      ...
      message.nRecipCount                       = nRecipients - (int) m_from.size(); // don't count originator
      message.lpRecips                          = message.nRecipCount ? &pRecipients[m_from.size()] : NULL;
