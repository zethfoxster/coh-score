...
inline int CString::Replace(LPCTSTR lpszOld, LPCTSTR lpszNew)
{
	...
	while (lpszStart < lpszEnd)
	{
		...
		{
			nCount++;
			lpszStart = (LPTSTR)(lpszTarget + nSourceLen);
		}
	...
	if (nCount > 0)
	{
		...
		while (lpszStart < lpszEnd)
		{
			while ( (lpszTarget = _cstrstr(lpszStart, lpszOld)) != NULL)
			{
				...
				memmove((void*)(lpszTarget + nReplacementLen), lpszTarget + nSourceLen, nBalance * sizeof(TCHAR));
				memcpy((void*)lpszTarget, lpszNew, nReplacementLen * sizeof(TCHAR));
				lpszStart = (LPTSTR)(lpszTarget + nReplacementLen);
...
// Attributes
	...
	BOOL GetFileTitle(LPTSTR lpstrFileTitle, int cchLength) const
	{
		...
		_tsplitpath_s(szBuff, NULL, 0, NULL, 0, szNameBuff, sizeof(szNameBuff), NULL, 0);
