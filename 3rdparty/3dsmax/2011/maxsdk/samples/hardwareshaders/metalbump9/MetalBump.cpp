...
#define GNORMAL_BITMAP_INDEX 2
#define GNORMAL_BUMP_AMNT 0
#define GNORMAL_PBLOCK_INDEX 0
...
static Texmap * SetUpNormalBumpTexmap(int index, Mtl * mtl, MaxShader * vs)
{
	...
	if(bTex)
	{
		bTex->GetParamBlock(GNORMAL_PBLOCK_INDEX)->GetValue(GNORMAL_BITMAP_INDEX,0,bmap,FOREVER);
		if(bmap == NULL)
		{
			bmap = NewDefaultBitmapTex();
			bTex->GetParamBlock(GNORMAL_PBLOCK_INDEX)->SetValue(GNORMAL_BITMAP_INDEX,0,bmap);
...
static void SetNormalBumpAmount(Mtl * mtl, int index, float bumpVal, TimeValue t)
{
	...
	if(bTex)
	{
		...
		bTex->GetParamBlock(GNORMAL_PBLOCK_INDEX)->SetValue(GNORMAL_BUMP_AMNT,t,bumpVal);
	}
}
...
void MaxShader::OverideMaterial()
{
   ...
   if(bumpOn)
   {
      ...
      if((m_VS->m_ChannelName[CHANNEL_BUMP].Length() && m_VS->m_ChannelName[CHANNEL_BUMP]!=TSTR(GetString(IDS_BITMAP_NONE)))
		  || (m_VS->m_ChannelName[CHANNEL_NORMAL].Length() && m_VS->m_ChannelName[CHANNEL_NORMAL]!=TSTR(GetString(IDS_BITMAP_NONE))))
      {
         int amnt=0;
         float bump;
         Control * cont = m_PBlock->GetController(SPIN_BUMPSCALE);
		 bool bNormalBump = m_VS->m_ChannelName[CHANNEL_NORMAL]!=TSTR(GetString(IDS_BITMAP_NONE)) && normal ? true : false;
		 BitmapTex *pTex = NULL;
