      ...
      while ((inchar = getc(fp)) != EOF) {
         ...
         if ((inchar == 'A') &&
            ...
            if (hlist) {
               ...
               item.cchTextMax = static_cast<UINT>(strlen(item.pszText)+1);   // SR DCAST64: Downcast to 4G limit.
    ...
    case WM_INITDIALOG: {
      ...
      for (int i = 0; i < bookmarks.Count(); i++) {
         ...
         item.lParam = (LPARAM)bookmarks[i];
            ...
            item.cchTextMax = static_cast<int>(strlen(item.pszText)+1); // SR DCAST64: Downcast to 2G limit.
