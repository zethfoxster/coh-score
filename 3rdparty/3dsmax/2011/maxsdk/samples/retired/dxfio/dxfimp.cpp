...
void NamedObjects::DeleteAll() {
	...
	count = 0;
#ifdef DBGNO
DebugPrint(_T("Deleted all named objects\n"));
#endif
	}
...
//		return -1;
	...
	count++;
#ifdef DBGNO
DebugPrint(_T("Added named object #%d: [%s], type %d, color %d\n"),count, name, type, color);
#endif
	return count-1;
	}
...
void NamedObjects::Delete(int index) {
	...
	delete ptr;
#ifdef DBGNO
DebugPrint(_T("Deleted named object #%d\n"),index);
#endif
	}
...
#ifdef DBGDXF
	DebugPrint(_T(" put_vert # %d, = %.7f,%.7f,%.7f \n"), i, v.x,v.y,v.z);
#endif
	t->verts[i] = v;
	}
...
#ifdef DBGDXF2
	DebugPrint(_T(" put_face # %d, = (%d,%d,%d)\n"), i, f.v[0],f.v[1],f.v[2]);
#endif
	t->faces[i] = f;
	}
...
static short inent(void) {
	...
	if ( ((++entnum)%10)==0) {
		...
		if (dxfobj>=0) {
			...
			nf = t->faces;
			strcpy(oname,n->name);
//			DebugPrint(_T(" Entity #%5d:%8s  Object \")%8s\"  V:%5d  F:%5d  #obs:%3d    \r",
//				entnum++,ename, oname,nv,nf,OBJECTS);
			}
//		else 
//			DebugPrint(_T(" Entity #%5d: %8s          \r"),entnum++,ename);
		}
...
#ifdef DBGCOL
	DebugPrint(_T(" INENT: layer = %s, color = %d \n"),tgroup[8],igroup[62]);
#endif
...
#ifdef DBGCOL
		DebugPrint(_T(" Inheriting current layer = %s \n"),curLayer);
#endif
		}
...
#ifdef DBGCOL 
			DebugPrint(_T(" inheriting BLOCK color = %d \n"),igroup[62]);
#endif
			}
		else 
			igroup[62] = 7;
...
#ifdef DBGCOL 
			DebugPrint(_T(" INENT:inheriting LAYER (%s) color = %d \n"),tgroup[8],igroup[62]);
#endif
			}
		}

#ifdef DBGCOL 
  	DebugPrint(_T(" FINAL color = %d \n"),igroup[62]);
#endif
		...
		if (Read(fp,&b, 1)==1) { 
			*type = (short) b;
			if (b==0xff) {
				if (Read(fp,type,2)==2) {
#ifdef DBGDXF
					DebugPrint(_T(" Extended entity %d \n"),*type);
#endif
					return(TRUE);
					}
				else
...
#ifdef DBGINGR
	  	DebugPrint(_T(" Binary INGROUP, gcode = %d "),gcode);
#endif
		if (gcode < 0) { 
#ifdef DBGINGR
			DebugPrint(_T(" Negative gcode: %d \n"), gcode);
#endif
...
//#endif
		...
		//else if (IR(80,90)||IR(280,290)) {
			...
			Read(fp,&cdum,1);
#ifdef DBGINGR
		  	DebugPrint(_T(" char = %x \n"), cdum);
#endif
			}					 
		else {              /* unknown gcode */
#ifdef DBGINGR
	  		DebugPrint(_T(" unknown gcode = %d \n"),wcode);
#endif
//SS 11/15/2000: Since we aren't able to read past the data, we have
// to error here; we can't just ignore it. The DXF import has no choice 
...
#ifdef DBGINGR
			DebugPrint(_T("ASCII INGROUP, gcode = %d /"),gcode);
#endif
			wcode=gcode;
...
#ifdef DBGDXF
	DebugPrint(_T(" exit INBLOCK, addr = %X, name =%s \n"),bl->fileAddr,bl->name);
	prblocks();
#else
#ifdef IS_3DSDXF
	DebugPrint(_T(" Block read: %s        \r"),bl->name);
#endif
#endif
	return TRUE;
	}
...
#ifdef DBGDXF
	DebugPrint(_T("load_vert, vert = (%.5f, %.5f %.5f)\n"),v.x,v.y,v.z);
#endif
	}
...
#ifdef DBGDXF
	DebugPrint(_T(" VERT: (%.3f, %.3f, %.3f): ptct=?? isSpline=%d\n"),rgroup[10],rgroup[20],rgroup[30],/*ptct,*/isSpline);
#endif
	if (dxf_type==DXF3D) {
		/* ---- 3D --------- */
		if(inShape) {
...
#ifdef DBGDXF
		DebugPrint(_T("*********** STARTING OBJECT: %s \n"), name);
#endif
#ifndef IS_3DSDXF
//		dxf_ob_prompt(name);
#endif
...
#ifdef DBGDXF
	  	DebugPrint(_T(" FACE3D vertex = (%.4f,%.4f,%.4f) \n"),v1.x,v1.y, v1.z);
#endif
...
static int AddVertsFaces(int nv, int nf) {
	int res;
#ifdef DBGDXF
	DebugPrint(_T("AddVertsFaces, nv=%d, nf = %d \n"),nv,nf);
#endif
	if (nv > 0) { 
...
static void PutAFace(int a, int b, int c, int smg, int fl) {
	...
	f.setMatID(get_mtl_id());
#ifdef DBGDXFFACES
	DebugPrint(_T("PutAFace %d,%d,%d (dxfverts=%d) mtl=%d\n"),a,b,c,dxfverts,f.getMatID());
#endif
...
static int put_quad(Point3 *p) {
	...
	for (i=0; i<4; i++) {
		v = p[i];
#ifdef DBGDXF
		DebugPrint(_T(" putquad: vert = %.4f,%.4f,%.4f \n"),v.x,v.y,v.z);
#endif
	 	ApplyTransform(v); 
...
#ifdef DBGDXF
	DebugPrint(_T(" CompEntityTransform: isEntityTM= %d:  norm = %.4f,%.4f,%.4f\n"),
...
#ifdef DBGDXF
DebugPrint(_T("Got arc %.4f to %.4f\n"),astrt,aend);
#endif
...
#ifdef DBGDXF1
	DebugPrint(_T(" DoBulge, ws= %4f, we = %4f\n"),ws,we);
#endif
	wstrt = ws;
	wend = we;
...
#ifdef DBGDXF1
	DebugPrint(_T(" cx = %.3f, cy = %.3f, radius = %.3f, ang0=%3f, angstep=%3f, nsteps =%d\n"),
...
static void IntsctSegments(PLPoint *p1, PLPoint *p2) {
	...
	if (!Int2PntLines(p1->b0,p1->b1,p2->b0,p2->b1, p2->bi))
		p2->bi = p2->b0;
#ifdef DBGDXF
	DebugPrint(_T(" IntsctSegs: (%.3f,%.3f,%.3f) (%.3f,%.3f,%.3f) \n"),
...
#ifdef DBGDXF1
	DebugPrint(_T(" AddPlineVert:  nplverts=%d, polyClosed = %d, isThick = %d\n"),nplverts,polyClosed,isThick);
	DebugPrint(_T("    0 = (%.3f,%.3f,%.3f) \n"),p[0],p[1],p[2]);
#endif
	plp.p = p;
	plp.wstrt = wstart;
...
static int FinishWidePline(void) {
	...
	int res = 1;
#ifdef DBGDXF1
	DebugPrint(_T("FinishWidePline  n = %d, closed = %d \n"),n,polyClosed);
#endif
	for (i=0; i<n; i++) {
		pv = PV_EL(i);
		if (lastb!=0) {
...
static int pline(void) {
...
#ifdef DBGDXF
	DebugPrint(_T(" PLINE: normal = %.4f, %.4f, %.4f \n"), normal[0],normal[1],normal[2]);
#endif
	...
	if (dxf_type==DXF3D) {
		...
		if(igroup[70] & 64)	{ /* polyface mesh */
			...
			if(find_dxfobj(DXF_MESH)==0)
				return(1);		/* Errored! */
#ifdef DBGDXF
			DebugPrint(_T(" PLINE:  PolyFace mesh\n"));
#endif
			inrat=TRUE;
			}
		else
...
#if 0
				DebugPrint(_T("    Null Poly mesh: grp[70] =%X  grp[75] = %X  Ncount = %d  Mcount = %d \n"),
...
#ifdef DBGDXF
			DebugPrint(_T(" PLINE:  3D Polygon mesh\n"));
#endif
	...
	/* Set flags which tell what to do with verts */
			...
			if ((polyClosed&&DXFImport::fillPolylines)||rgroup[39]!=0.0||rgroup[40]!=0) { 
				...
				if (find_dxfobj(DXF_MESH)==0) return(1);	
#ifdef DBGDXF
				DebugPrint(_T(" PLINE: As 3d solid: closed=%d  thick =%.3f, wide=%.3f\n"), polyClosed, rgroup[39],rgroup[40]);
#endif
				SetThick();
				lastbulge=0.0f;
				nplverts=0;
				...
				if (find_dxfobj(DXF_SHAPE)==0)
					return(1);	
#ifdef DBGDXF
				DebugPrint(_T("Started pline shape, closed=%d\n"),polyClosed);
#endif
...
#ifdef DBGDXF
	DebugPrint(_T(" SEQEND: dxf_type = %d, inpoly=%d anyWide=%d  inrat=%d closed=%d\n"),
		...
		if (dxf_type==DXF2D||(dxf_type==DXFPATH&&!isSpline))	{
			...
			if (isSpline) {
				if (COUNT(splineTab)>0) {
					...
					DebugPrint(_T("\n\nSPLINE: polyClosed = %d\n"),polyClosed);
					for (i=0; i<count; i++) {
						DebugPrint(_T(" point[%d] = (%2f,%.2f,%.2f) \n"),i,sp[i].x,sp[i].y,sp[i].z);
						}
					DebugPrint(_T("\n"));
					}
#endif
...
#ifdef IS_3DSDXF
		DebugPrint(_T(" Couldn't find block %s\n"), tgroup[2]);
#endif
		return(0);
		}
	
#ifdef DBGDXF1
	DebugPrint(_T(" INSERT, level# %d,  block %s, layer = %s,gtextv=%s \n"), blockLevel,tgroup[2], tgroup[8],gtextv);
#endif
...
#ifdef  DBGCOL 
	DebugPrint(_T(" BlockCol = %d \n"), curBlockCol);
#endif

#ifdef  DBGDXF1 
	DebugPrint(_T(" InsPoint = (%.3f,%.3f,%.3f) norm=(%.3f,%.3f,%.3f)\n"),
		inspt[0],inspt[1],inspt[2],norm[0],norm[1],norm[2]);
	DebugPrint(_T(" Angle = %.4f, Scaling= (%.3f,%.3f,%.3f)\n"),
...
#ifdef  DBGDXF1 
			DebugPrint(_T(" basePoint = (%.3f,%.3f,%.3f) \n"),
...
#ifdef DBGDXF1
	DebugPrint(_T(" LEAVING INSERT, level# %d,gtextv=%s,ename=%s\n\n"), blockLevel,
		gtextv,ename);
#endif
bail:
	...
	while (!errored && inent())	{
		if(ignore_layer(tgroup[8])||igroup[67])	   {
...
#ifdef DBGDXF1
			if (igroup[67])
				DebugPrint(_T(" Ignoring PAPER SPACE entity %s, layer =%s \n"),ename,tgroup[8]);
			else 
				DebugPrint(_T(" Ignoring entity on OFF LAYER %s \n"),tgroup[8]);
#endif
			}
		else {
#ifdef DBGDXF
			DebugPrint(_T(" Processing %s entity layer = %s, \n"),ename,tgroup[8]);
#endif
			...
			if (!strcmp(ename, "ATTRIB")) {	}
	  			...
	  			DebugPrint(_T("     Unknown Entity: %s \n"), ename);
				}
#endif
			}
		}
...
if(finalVerts != origVerts)
	DebugPrint(_T("Weld dropped vert count to %d from %d\n"),finalVerts,origVerts);
#endif // DBGFINAL
	}
...
#ifdef DBGFINAL
				DebugPrint(_T("     Object %s created, %d Verts,  %d Faces\n"), n->name, verts, faces);
#endif
				if(verts || faces) {
					if (theImport->weld) {
#ifdef DBGFINAL
						DebugPrint(_T("     		Welding (%f)..."), theImport->thresh);
#endif
						WeldVerts(m, theImport->thresh);

#ifdef DBGFINAL
						DebugPrint(_T("    %d Verts,  %d Faces\n"), m->getNumVerts(), m->getNumFaces());
#endif
						}
					if (theImport->weld || theImport->removeDouble) {
#ifdef DBGFINAL
						DebugPrint(_T("     		Removing double faces..."));
#endif
						RemoveDoubleFaces(m);
...
#ifdef DBGFINAL
						DebugPrint(_T("     		Unifying Normals...\n"));
#endif
	...
	if(status == 0)
		...
		if(status > 0)	{
			if(!dxf_finalstep(iface))
				DebugPrint(_T("Error loading into MAX\n"));
			}
		}
