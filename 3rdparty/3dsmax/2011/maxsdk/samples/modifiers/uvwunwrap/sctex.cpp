...
UBYTE *RenderTexMap(Texmap *tex,int w, int h, BOOL useAlpha)
	{
...
//	UBYTE *image = new UBYTE[ByteWidth(w*3)*h];
	...
	for (int j=0; j<h; j++) {
		...
		for (int i=0; i<w; i++) {
			...
			*p1++ = idispGamMgr->ColorCorrect8((UBYTE)(col.b*255.0f),IColorCorrectionMgr::kBLUE_C);
			*p1++ = idispGamMgr->ColorCorrect8((UBYTE)(col.g*255.0f),IColorCorrectionMgr::kGREEN_C);
			*p1++ = idispGamMgr->ColorCorrect8((UBYTE)(col.r*255.0f),IColorCorrectionMgr::kRED_C);	
			sc.uvw.x +=du;
			}		
		sc.uvw.y -= dv;
		}
