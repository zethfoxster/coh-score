...
class Color64U : public BMM_Color_64
...
class Color32F : public BMM_Color_fl
...
public:
	Color32F() { }
	Color32F( float R, float G, float B, float A ){ r=R; g=G; b=B; a=A; }
	Color32F& operator+=(const Color32F& c){ r += c.r ; g += c.g; b += c.b; a += c.a; return *this; }
	Color32F& operator*=(const float f){ r *= f; g *= f; b *= f; a *= f; return *this; }
	void AddSafe( const Color32F& c )
	{
		// The normal addition is safe with floating-point values
		*this += c;
	}
};
...
private:
	...
	void BlendImages_Internal( Bitmap* pDestBM, Bitmap* pSrcBM, int nImage );
};
