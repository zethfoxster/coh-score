...
void BlendTable::BlendImages( Bitmap* pDestBM, Bitmap* pSrcBM, int nImage ) {

	// Use a different color type depending on the bitmap type
	if(pDestBM->IsHighDynamicRange()) {
		BlendImages_Internal<Color32F>(pDestBM, pSrcBM, nImage);
	}
	else {
		BlendImages_Internal<Color64U>(pDestBM, pSrcBM, nImage);
	}
}

template <class ColorType>
void BlendTable::BlendImages_Internal( Bitmap* pDstBM, Bitmap* pSrcBM, int nImage )
{
	// get xSz & ySz
	...
	// get line buffers
	ColorType *pOut = new ColorType[ xSz ];
	ColorType *pIn = new ColorType[ xSz ];

	// for each line of the image...
	for( int nLine = 0; nLine < ySz; ++nLine ){

		// get src & dst lines
		pDstBM->GetPixels( 0, nLine, xSz, pOut );
		pSrcBM->GetPixels( 0, nLine, xSz,  pIn );
		...
		pDstBM->PutPixels(0, nLine, xSz,  pOut );
