...
_TIFFVSetField(TIFF* tif, ttag_t tag, va_list ap)
{
    ...
    case TIFFTAG_RICHTIFFIPTC: 
		...
		case TIFFTAG_MODELPIXELSCALE:
			_TIFFsetDoubleArray(&td->td_modpixelscale, va_arg(ap, double*), 3);
			break;
		case TIFFTAG_MODELTRANS:
			_TIFFsetDoubleArray(&td->td_modtrans, va_arg(ap, double*), 16);
			break;
		case TIFFTAG_MATRIX:
			_TIFFsetDoubleArray(&td->td_matrix, va_arg(ap, double*), (int32)va_arg(ap, int));
			break;
		case TIFFTAG_MODELTIES:
			td->td_modtiescount = (uint16) va_arg(ap, int);
			_TIFFsetDoubleArray(&td->td_modties, va_arg(ap, double*), td->td_modtiescount);
			break;
		case TIFFTAG_GEOKEYDIR:
			td->td_geokeydir = (uint16*)va_arg(ap, int *);
			break;
	...
	default:
		...
		TIFFError("TIFFSetField",
		    ...
		    tif->tif_name, isPseudoTag(tag) ? GetString(IDS_PSEUDO) : "",
...
OkToChangeTag(TIFF* tif, ttag_t tag)
{
	...
	if (!fip) {			/* unknown tag */
		TIFFError("TIFFSetField", GetString(IDS_UNKNOWN_TAG),//"%s: Unknown %stag %u"
		    tif->tif_name, isPseudoTag(tag) ? GetString(IDS_PSEUDO) : "", tag);
		return (0);
	}
...
#ifdef GEOREFSYS_UVW_MAPPING 
	/* Support for GeoTiff in 3DStudio VIZ */
	td->td_modpixelscale = NULL;
	td->td_matrix = NULL;
	td->td_modtiescount = 0;
	td->td_modties = NULL;
	td->td_modtrans = NULL;
	td->td_geokeydir = NULL;
	td->td_geodouble = NULL;
	td->td_geoascii = NULL;
#endif 
