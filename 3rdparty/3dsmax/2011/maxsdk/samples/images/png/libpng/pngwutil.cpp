   ...
   png_int_32 X1, int type, int nparams, png_charp units, png_charpp params)
{
   ...
   for (i = 0; i < nparams; i++)
   {
      params_len[i] = static_cast<png_uint_32>(png_strlen(params[i]) + (i == nparams - 1 ? 0 : 1));	// SR DCAST64: Downcast to 4G limit.
      png_debug2(3, "pCAL parameter %d length = %d\n", i, params_len[i]);
      total_len += (png_size_t)params_len[i];
   }

   png_debug1(3, "pCAL total length = %d\n", total_len);
   png_write_chunk_start(png_ptr, png_pCAL, static_cast<png_uint_32>(total_len));	// SR DCAST64: Downcast to 4G limit.
   png_write_chunk_data(png_ptr, new_purpose, static_cast<png_uint_32>(purpose_len));	// SR DCAST64: Downcast to 4G limit.
