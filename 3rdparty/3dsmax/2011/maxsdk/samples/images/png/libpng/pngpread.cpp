...
png_push_read_sig(png_structp png_ptr, png_infop info_ptr)
{
   ...
   png_push_fill_buffer(png_ptr, &(info_ptr->signature[num_checked]), 
      num_to_check);
   png_ptr->sig_bytes += static_cast<png_byte>(num_to_check);	// SR NOTE64: Assumes signature is small
...
png_push_crc_finish(png_structp png_ptr)
{
   if (png_ptr->skip_length && png_ptr->save_buffer_size)
   {
      ...
      png_ptr->skip_length -= static_cast<png_uint_32>(save_size);	// SR DCAST64: Downcast to 4G limit.
      png_ptr->buffer_size -= static_cast<png_uint_32>(save_size);	// SR DCAST64: Downcast to 4G limit.
...
png_push_read_IDAT(png_structp png_ptr)
{
   ...
   if (png_ptr->idat_size && png_ptr->save_buffer_size)
   {
      ...
      png_ptr->idat_size -= static_cast<png_uint_32>(save_size);	// SR DCAST64: Downcast to 4G limit.
      png_ptr->buffer_size -= static_cast<png_uint_32>(save_size);	// SR DCAST64: Downcast to 4G limit.
      png_ptr->save_buffer_size -= save_size;
      png_ptr->save_buffer_ptr += save_size;
   }
...
png_push_read_zTXt(png_structp png_ptr, png_infop info_ptr)
{
   ...
   if (!(png_ptr->current_text_left))
   {
      ...
      png_ptr->zstream.avail_out = static_cast<uInt>(png_ptr->zbuf_size);	// SR DCAST64: Downcast to 4G limit.
