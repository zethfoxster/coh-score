...
void AudioP3Dlg::Update()
{
   if (!valid && hWnd) {
      ...
      if (!fileName.isNull())
         SendMessage(GetDlgItem(hWnd, IDC_AUDAMP_FILENAME), WM_SETTEXT, 0, (LPARAM)fileName.data());
      else
   ...
   switch (id) {
      ...
      case IDC_BTN_BROWSE: {
            ...
            if (asset.GetId()!=kInvalidId) {
               ...
               SendMessage(GetDlgItem(hWnd, IDC_AUDAMP_FILENAME), WM_SETTEXT, 0, (LPARAM)asset.GetFileName().data());
               EnableChannelUI(cont->wave->GetNumChannels() == 2);
            }
            Change();
         }
