...
TCHAR* CompassRoseObject::GetObjectName() 
{ 
	return const_cast<TCHAR*>(MaxSDK::GetResourceString(IDS_DB_COMPASS_OBJECT));
}
...
void CompassRoseObject::BeginEditParams(
   ...
   if (!hParams) {
      hParams = ip->AddRollupPage( 
            ...
            PointParamProc, 
            const_cast<TCHAR*>(MaxSDK::GetResourceString(IDS_DB_PARAMETERS)), 
            (LPARAM)this );
...
CreateMouseCallBack* CompassRoseObject::GetCreateMouseCallBack() {
   mCompassRoseCreateCB.SetObj(this);
   return(&mCompassRoseCreateCB);
   }
...
// vpt->getGW()->setRndLimits( limits & ~GW_Z_BUFFER );  JH
   ...
   Text( vpt, MaxSDK::GetResourceString(IDS_EAST_MARK), Point3(corners,0.0f,0.0f) ); 
   Text( vpt, MaxSDK::GetResourceString(IDS_WEST_MARK), Point3(-corners,0.0f,0.0f) ); 
   Text( vpt, MaxSDK::GetResourceString(IDS_NORTH_MARK), Point3(0.0f,corners,0.0f) ); 
   Text( vpt, MaxSDK::GetResourceString(IDS_SOUTH_MARK), Point3(0.0f,-corners,0.0f) ); 
