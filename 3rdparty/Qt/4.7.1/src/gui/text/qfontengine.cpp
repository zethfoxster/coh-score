...
quint32 QFontEngine::getTrueTypeGlyphIndex(const uchar *cmap, uint unicode)
{
    ...
    } else if (format == 10) { // added to support OSX Lion Fonts
        quint32 tableSize = qFromBigEndian<quint32>(cmap + 4);

        quint32 firstCode10 = qFromBigEndian<quint32>(cmap + 12);
        if (unicode < firstCode10)
            return 0;

        quint32 entryCount10 = qFromBigEndian<quint32>(cmap + 16);
        if (entryCount10 * 2 + 20 > tableSize)
            return 0;

        quint32 sentinel10 = firstCode10 + entryCount10;
        if (unicode >= sentinel10)
            return 0;

        quint32 entryIndex10 = unicode - firstCode10;
        return qFromBigEndian<quint16>(cmap + 20 + (entryIndex10 * 2));
    } else if (format == 12) {
